...
def backtest_single_season(
    ...
    if calibrators is not None:
        ...
        if temperature is not None:
            ...
            predictions_cal = apply_calibration(predictions_uncal, calibrators)
            ...
            # determine what type of calibration was applied
            cal_type = "standard"
            if isinstance(temperature, dict):
                cal_type = "outcome-specific"
                temp_str = f"T_h={temperature.get('T_home', 1):.2f}, T_d={temperature.get('T_draw', 1):.2f}, T_a={temperature.get('T_away', 1):.2f}"
            else:
                temp_str = f"T={temperature:.3f}"

            if verbose:
                print(f"\nCalibrated Results ({cal_type}: {temp_str}):")
