...
def fit_temperature_scaler(
    ...
    """
    Fit temperature scaling parameter via RPS minimisation.
    ...
    # pre-compute one-hot actuals for rps calculation
    actual_one_hot = np.zeros((len(actuals), 3))
    actual_one_hot[np.arange(len(actuals)), actuals] = 1.0

    def mean_rps(temperature: float) -> float:
        """Compute mean RPS with given temperature"""
        ...
        # inline rps: cumulative squared differences, normalised by (K-1)=2
        cum_pred = np.cumsum(calibrated_probs, axis=1)
        cum_actual = np.cumsum(actual_one_hot, axis=1)
        return float(np.mean(np.sum((cum_pred - cum_actual) ** 2, axis=1) / 2))

    # optimise temperature (search range: 0.1 to 5.0)
    result = minimize_scalar(mean_rps, bounds=(0.1, 5.0), method="bounded")

    optimal_temperature = result.x

    if verbose:
        uncalibrated_rps = mean_rps(1.0)
        calibrated_rps = result.fun

        print("   Temperature optimisation:")
        print(f"     Optimal T: {optimal_temperature:.3f}")
        print(f"     Uncalibrated RPS: {uncalibrated_rps:.4f}")
        print(f"     Calibrated RPS: {calibrated_rps:.4f}")
        print(f"     Improvement: {uncalibrated_rps - calibrated_rps:.4f}")
