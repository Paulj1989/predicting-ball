...
def parse_args():
    ...
    parser.add_argument(
        ...
        default="rps",
        help="Primary metric to report and emphasize (default: rps)",
    )
...
def main():
    ...
    for result in results:
        ...
        baseline_info = ""
        if result.get("baseline_metrics") and args.metric in ["rps", "log_loss", "brier"]:
            ...
            if baseline_key in result["baseline_metrics"]:
                ...
                if isinstance(primary_value, (int, float)) and isinstance(baseline_value, (int, float)):
        ...
        if args.metric == "rps":
            print(f"  Season {season}: RPS={rps:.4f} (PRIMARY), Brier={brier if brier is not None else 'N/A'}, Log Loss={log_loss if log_loss is not None else 'N/A'}{baseline_info}")
        elif args.metric == "log_loss":
            print(f"  Season {season}: Log Loss={log_loss if log_loss is not None else 'N/A'} (PRIMARY), Brier={brier if brier is not None else 'N/A'}, RPS={rps:.4f}{baseline_info}")
        else:  # brier
            print(f"  Season {season}: Brier={brier if brier is not None else 'N/A'} (PRIMARY), RPS={rps:.4f}, Log Loss={log_loss if log_loss is not None else 'N/A'}{baseline_info}")
    ...
    avg_brier = np.mean([r["metrics"].get("brier_score", 0) for r in results if r["metrics"].get("brier_score") is not None])
    avg_log_loss = np.mean([r["metrics"].get("log_loss", 0) for r in results if r["metrics"].get("log_loss") is not None])
