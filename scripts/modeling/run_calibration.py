...
def main():
    ...
    from src.evaluation.metrics import calculate_brier_score, calculate_rps, calculate_log_loss
    ...
    # emphasize selected metric with improvement
    if args.metric == "rps":
        rps_uncal = metrics_uncal_cal.get('rps', rps_cal)
        print(f"     RPS: {rps_cal:.4f} (Δ = {rps_cal - rps_uncal:+.4f}) (PRIMARY)")
        print(f"     Brier: {brier_cal:.4f} (Δ = {brier_cal - metrics_uncal_cal['brier_score']:+.4f})")
        print(f"     Log Loss: {log_loss_cal:.4f}")
    elif args.metric == "log_loss":
        log_loss_uncal = metrics_uncal_cal.get('log_loss', log_loss_cal)
        print(f"     Log Loss: {log_loss_cal:.4f} (Δ = {log_loss_cal - log_loss_uncal:+.4f}) (PRIMARY)")
        print(f"     Brier: {brier_cal:.4f} (Δ = {brier_cal - metrics_uncal_cal['brier_score']:+.4f})")
        print(f"     RPS: {rps_cal:.4f}")
    else:  # brier
        print(f"     Brier: {brier_cal:.4f} (Δ = {brier_cal - metrics_uncal_cal['brier_score']:+.4f}) (PRIMARY)")
        rps_uncal = metrics_uncal_cal.get('rps', rps_cal)
    ...
    # emphasize selected metric first
    if args.metric == "rps":
        ...
        print(f"  Log Loss improvement: {holdout_metrics.get('log_loss_improvement', 'N/A')} (PRIMARY)")
        print(f"  Brier improvement: {holdout_metrics['brier_improvement']:+.4f}")
